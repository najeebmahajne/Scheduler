...
typedef enum 
{
  ...
  SORTED_GENERAL_ERR = 2
} sorted_list_status_ty;
...
in success : return the previous 
on failure: NULL.
...
Undefined behaviour: if list, compare function or element, output are NULL pointers.
...
*/
sorted_list_iter_ty SortedListFindIf(sorted_list_iter_ty from,sorted_list_iter_ty
                                   to ,cmp_func_ty cmp_func ,const void *param);

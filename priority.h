...
Undefined behaviour: p_list is NULL

Time Complexity: O(n)

on failure: None.
*/
void PriorityDestroy(priority_ty *p_list);
...
on failure: priority_queue_status_ty -PRIORITY_MALLOC_ERROR(1) 
                                    or PRIORITY_GENERAL_ERR(2).
*/
priority_queue_status_ty PriorityEnqueue(priority_ty *p_list,void *data);
...
on failure: None.
*/
void *PriorityDequeue(priority_ty *p_list);
...
on failure: None.
*/
void *PriorityPeek(priority_ty *p_list);
...
on failure: None.
*/
int PriorityIsEmpty(const priority_ty *p_list);
...
on failure: None.
*/
size_t PrioritySize(const priority_ty *p_list);
...
on failure: None.
*/
void PriorityClear(priority_ty *p_list);
...
*/
void *PriorityErase(priority_ty *p_list,cmp_func_ty is_match, void *data);
